...
@pytest.mark.db(serialized_rollback=True)
...
# Show that serialized_rollback is NOT working as expected to restore the database after each test
@pytest.mark.db(serialized_rollback=True)
def test_that_serialized_rollback_restores_migrated_data(polls_page):
    polls_page.navigate()

    # This fails, the database has been flushed and the data from the migration is gone
    expect(polls_page.page.get_by_text('What is your favorite holiday?')).to_be_visible()
